...
const EditSecondStep = () => {
    ...
    const submit = async (data) => {
        await ctx.WorkoutStore.update(ctx.WorkoutStore.workout.workout_id, {
    ...
    return (
        <div className={classes['create-workout']}>
            {ctx.WorkoutStore?.workout?.exercises &&
                <>
                    ...
                    <form method='post' onSubmit={handleSubmit(submit)} className={classes['create-workout__form']}>
                        {ctx.WorkoutStore.workout.exercises.map((exercise, index) =>
                            <div>
